module.exports = {
    ...
    clientID: process.env.clientID || '在此輸入或使用.env', // 機器人註冊指令 (/) 會使用這個機器人ID (填入機器人ID 或者Developer Portal的Application ID)
    ...
    inviteChannel: process.env.inviteChannel || '在此輸入或使用.env', // 機器人的進出伺服器會記錄在這個頻道 (填入ID)
    ...
    slashcmdreg: process.env.slashcmdreg || true, // 第一次啟動請用true，否則請用false(清空字串)，如果為true則會註冊機器人互動應用程式指令 (/)
    CI: true, // 請普通用戶設爲false
};
