...
const startbot = (config) => {
    ...
    // 回覆或偵測用文字
    const bullshitter = [
        '世界上的所有人都需要<name>，因為他們的生活中無時無刻都有著<name>的存在，要是沒有<name>，世界可能會就此滅絕。<name>第一次被人類發現是在今年一月一日，並取名為<name>，並把當日訂為一個重要的日子。為了讓大家不會缺乏<name>，聯合國特別訂定了<name>使用限制法。',
        '每當我使用<name>時，我總會感到神奇，因為我很好奇為什麼<name>會那麼好用，讓我無時無刻都想使用他。我尚未開始使用<name>時，我的生活非常不方便；自從我開始使用<name>後，再也沒有煩惱產生了，你一定要用用看<name>！',
        '<name>是一個複雜的議題，幾乎沒有人可以使用隻字片語解釋他。<name>的深奧有如微積分一般複雜，目前世界上至今無人解釋出<name>的內涵意義。',
        '<name>，一個目前尚未得知原因的疾病，目前有許多科學家正在研究<name>，因為鉛洗年曾經公布研究<name>成功的人可得到三千萬獎金。但目前依舊沒有科學家成功得到<name>的答案。全世界已有數千萬人得到<name>疾病，若不根除，必定讓人類滅亡。',
        '為什麼我們需要<name>呢？這牽涉到了數百年前金屬器時代的一個問題，當時台灣尚未開始發展，但因為<name>的出現，解決了人們日常生活中的各種問題。<name>可以從許多管道取得，像是百貨公司、學校等生活處所，且不用任何成本即可使用。',
        '有一天，我和家人出遊，途中看到了一些<name>，於是我很好奇，為什麼<name>會存在於世界上呢？經過我查詢資料後發現，<name>會出現在世界中是因為我們的製造，人類經過不斷的研發，才研究出了<name>，讓大家的生活更加便利。',
    ];
    const reply = [
        '不知道',
        '我覺得可以喔',
    ...
    const nosafe = [
        'https://discordgift.site/Q4YFjxxdkms2kzIo',
        'discocrd.com',
        'discord-egift.com',
        'https://discords-premium.com/',
        'https://dlscord.org/nitro-gift',
        'http://discordc.gift/Yexjh7BuKC',
        'https://minecraftspring04.weebly.com/java7',
        'ɢoogle.com',
    ];
    const eatcheap = [
        '蝦仁炒飯',
    ...
    const sayinvite = new MessageEmbed()
        ...
        .setFooter(
            `${config.botName}`,
            'https://cdn.discordapp.com/avatars/891195320690700299/7e70c4d7db63c7466488c8e9c6319307.png?size=80',
        );
    ...
    // leave('要離開的伺服器id')
        ...
        console.log(
            chalk.blue('啟動通知 ') + `${client.user.tag} 已成功登入並上線！`,
        );
        ...
        // 終端紀錄
        console.log(
            chalk.blue('啟動通知 ') + `${client.guilds.cache.size} 個伺服器`,
        );

        // 設定狀態

        const all = client.guilds.cache.reduce(
    ...
    client.on('messageDelete', (message) => {
        ...
        const sayre = new MessageEmbed()
            ...
            .setTitle('審核日誌')
            .setDescription(
                `一則由 ${message.author.tag} 發送的訊息遭到刪除\n內容：${message.content}`,
            )
            .setFooter(
                `${config.botName}`,
                'https://cdn.discordapp.com/avatars/891195320690700299/7e70c4d7db63c7466488c8e9c6319307.png?size=80',
    ...
    client.on('interactionCreate', async (interaction) => {
        ...
        commandChannel.send(
            '```' +
        `【${interaction.guild.name}】：[${interaction.user.tag}]使用了( /${interaction.commandName} )指令` +
        '```',
        );
        ...
        if (interaction.commandName === 'eatwhat') {
            try {
                ...
                if (money > 1100) {
                    await interaction.reply(
                        '今晚，我想來點：' + eatsoexp[getRandom(eatsoexp.length)],
                    );
                } else if (money > 500) {
                    await interaction.reply(
                        '今晚，我想來點：' + eatexpen[getRandom(eatexpen.length)],
                    );
                } else if (money > 200) {
                    await interaction.reply(
                        '今晚，我想來點：' + eatnomal[getRandom(eatnomal.length)],
                    );
                } else if (money > 20) {
                    await interaction.reply(
                        '今晚，我想來點：' + eatcheap[getRandom(eatcheap.length)],
                    );
                } else {
                    await interaction.reply(
                        '你擁有的錢過少，機器人也不知道你能吃什麼了。',
                    );
                }
            } catch (error) {
                interaction.reply('🪀 機器人端發生錯誤，請檢查指令或進行回報');
                    ...
                    manager.on('end', (next) => {
                        if (next) {
                            if (next.details.from === 'Youtube') {
                                res
                                    .setDescription(
                                        `開始播放 [${next.title}](${next.details.data.url})`,
                                    )
                                    .setThumbnail(next.details.data.thumbnailUrl)
                                    .setFooter(
                                        `由 ${next.player.displayName} 指定的歌曲`,
                                        next.player.user.displayAvatarURL(),
                                    );
                            } else {
                                res
                                    .setDescription(
                                        `開始播放 [${
                                            next.title === 'unknown' ? next.audioResource : next.title
                                        }](${next.audioResource})`,
                                    )
                                    .setThumbnail('')
                                    .setFooter(
        ...
        interaction.member.voice.channel.id !== manager.channel.id
            ...
            const afterPlay = async ([track, queued]) => {
                ...
                res.setFooter(
                    `由 ${track.player.displayName} 指定的歌曲`,
                    track.player.user.displayAvatarURL(),
                );

                if (queued) {
                    if (track.details.from === 'Youtube') {
                        res
                            .setThumbnail(track.details.data.thumbnailUrl)
                            .setDescription(
                                `已將 [${track.title}](${track.details.data.url}) 加入隊列`,
                            );
                    } else {
                    ...
                    res
                        .setThumbnail(track.details.data.thumbnailUrl)
                        .setDescription(
                            `開始播放 [${track.title}](${track.details.data.url})`,
                        );
                } else {
            ...
            if (YoutubeUtils.isYoutubeLink(query))
                try {
                    return afterPlay(
                        await manager.play(query, { player: interaction.member }),
                    );
                } catch (_) {
                ...
                try {
                    return afterPlay(
                        await (
                            await YoutubeUtils.searchFirstVideo(query)
                        ).play(manager, { player: interaction.member }),
                    );
                } catch (_) {
        ...
        if (interaction.commandName === 'nowplaying') {
            ...
            if (np.details.from === 'Youtube') {
                des =
          `正在播放：[${np.title}](${np.details.data.url})\n\n` +
          `播放時間：${timeResolve(~~(np.playedMs / 1000))} / ${timeResolve(
              +np.details.data.lengthSeconds,
          )}\n\n` +
          `上傳頻道：[${np.details.data.channel.name} ${
              np.details.data.channel.verified ? '☑️' : ''
          }](${np.details.data.channel.url}) \n\n` +
          `上傳日期：${np.details.data.uploadDate}\n\n` +
          `觀看次數：${np.details.data.viewCount.replace(
              /(.)(?=(\d{3})+$)/g,
              '$1,',
          )}\n\u200b`;
                ...
                des =
          `正在播放：[${
              np.title === 'unknown' ? np.audioResource : np.title
          }](${np.audioResource})\n\n` +
          `播放時間：${timeResolve(~~(np.playedMs / 1000))}`;
            ...
            res
                .setDescription(des)
                .setFooter(
                    `由 ${np.player.user.tag} 指定的樂曲`,
                    np.player.user.displayAvatarURL(),
                );
        ...
        if (interaction.commandName === 'queue') {
            ...
            const queue = manager.queue.map((track) => {
                const name =
          track.title === 'unknown' ? track.audioResource : track.title;
                const url =
          track.details.from === 'Youtube'
              ? track.details.data.url
              : track.audioResource;
                return `[${name}](${url})`;
            });
            ...
            const pages = [];
            const np = interaction.client.music.get(interaction.guild.id).nowPlaying;

            const nowPlaying = {
                name: np.title === 'unknown' ? np.audioResource : np.title,
                url:
          np.details.from === 'Youtube'
              ? np.details.data.url
              : np.audioResource,
            };
            ...
            res
                .setAuthor('音樂中心', interaction.client.user.displayAvatarURL())
                .setDescription(
                ...
                .setFooter(
                    `${interaction.user.tag}・第 ${index + 1}/${pages.length} 頁`,
                    interaction.user.displayAvatarURL(),
                );
        ...
        interaction.member.voice.channel.id !== manager.channel.id
            ...
            res.setDescription(
                `${interaction.user}，已移除編號在 ${start} 到 ${end} 之間的所有歌曲`,
            );
            if (start === end)
        ...
        if (interaction.commandName === 'list-server') {
            if (interaction.user.id == config.owner) {
                client.guilds.cache.forEach((guild) => {
                    console.log(
                        chalk.yellow('所在伺服 ') + `${guild.name} | ${guild.id}`,
                    );
                });
        ...
        if (interaction.commandName === 'help') {
            ...
            res = new MessageEmbed()
                ...
                .setFooter(
                    `${interaction.user.tag}・第 ${index + 1}/${pages} 頁`,
                    interaction.user.displayAvatarURL(),
                )
                .addFields({
        ...
        interaction.member.voice.channel.id !== manager.channel.id
            ...
            message
                ...
                .then((i) => {
                    ...
                    message.delete();
                    data
                        .play(interaction.client.music.get(interaction.guild.id), {
        ...
        if (interaction.commandName === 'together-words-snack') {
            if (!interaction.member.voice.channel)
                return interaction.reply('請先加入一個語音頻道');
            client.discordTogether
                .createTogetherCode(interaction.member.voice.channel.id, 'wordsnack')
                .then(async (invite) => {
                    return interaction.reply(`${invite.code}`);
                });
        }
        if (interaction.commandName === 'together-doodle-crew') {
            if (!interaction.member.voice.channel)
                return interaction.reply('請先加入一個語音頻道');
            client.discordTogether
                .createTogetherCode(interaction.member.voice.channel.id, 'doodlecrew')
                .then(async (invite) => {
                    return interaction.reply(`${invite.code}`);
                });
        }
        if (interaction.commandName === 'together-spellcast') {
            if (!interaction.member.voice.channel)
                return interaction.reply('請先加入一個語音頻道');
            client.discordTogether
                .createTogetherCode(interaction.member.voice.channel.id, 'spellcast')
                .then(async (invite) => {
                    return interaction.reply(`${invite.code}`);
                });
        }
        if (interaction.commandName === 'together-awkword') {
            if (!interaction.member.voice.channel)
                return interaction.reply('請先加入一個語音頻道');
            client.discordTogether
                .createTogetherCode(interaction.member.voice.channel.id, 'awkword')
                .then(async (invite) => {
                    return interaction.reply(`${invite.code}`);
                });
        }
        if (interaction.commandName === 'mora') {
            ...
            if (youcho == 'sci' && botcho == 'sto')
                return interaction.reply(
                    `你選擇：${a}；機器人選擇：${b}\n結果：您輸了`,
                );
            if (youcho == 'sci' && botcho == 'pap')
                return interaction.reply(
                    `你選擇：${a}；機器人選擇：${b}\n結果：您贏了`,
                );
            if (youcho == 'pap' && botcho == 'sto')
                return interaction.reply(
                    `你選擇：${a}；機器人選擇：${b}\n結果：您贏了`,
                );
            if (youcho == 'pap' && botcho == 'sci')
                return interaction.reply(
                    `你選擇：${a}；機器人選擇：${b}\n結果：您輸了`,
                );
            if (youcho == 'sto' && botcho == 'sci')
                return interaction.reply(
                    `你選擇：${a}；機器人選擇：${b}\n結果：您贏了`,
                );
            if (youcho == 'sto' && botcho == 'pap')
                return interaction.reply(
                    `你選擇：${a}；機器人選擇：${b}\n結果：您輸了`,
                );
        }
        if (interaction.commandName === 'bullshitter') {
            const bullreply = `${
                bullshitter[getRandom(bullshitter.length) - 1]
            }`.replace(/<name>/g, topic);
